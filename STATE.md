...
- **Normalize + validate pipeline (F.2) wired + run for Jan:** `tools/normalize_fixture.py` now takes `--year` (2027 → `anno_unified_2027.json`; 2026 default unchanged, 182 entries). Injects `mock_priority`, capitalizes ranks, EN/VI mutual fallback. Validators: `validate_vietnamese_integrity` CLEAN (0 problems); `validate_engine_b_output` **0 errors** (body gate relaxed ≥3→≥1 para to match devotional depth). 12 soft warnings = type↔URL-pattern on allowlist backfills (non-blocking).
- **Driver hardening:** `repair_vi` now passes EN context (ferial days returned empty VI without it) + reverse-fills missing `_en` from `_vi`; future months self-heal.
