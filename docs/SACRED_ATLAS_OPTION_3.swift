...
// MARK: - 1. LOCALIZATION
// ═══════════════════════════════════════════════════════════════
// All chrome strings pass through here. Content (prayers, narratives)
// stays in fixture data (English v0). Add vi.lproj/Localizable.strings
// to enable Vietnamese without touching code.
enum L {
    ...
    static let unknownHours      = LocalizedStringKey("anno.section.unknownHours")
    ...
    static let addToJourney      = LocalizedStringKey("anno.action.addToJourney")
...
final class PassportStamp {
    ...
    init(location: SacredLocation) {
        ...
        self.localizedStampCode = "SIGIL-\(Int.random(in: 1000...9999))-\(String(location.name.prefix(3).uppercased()))"
    }
    ...
    private init() {
        ...
        locations = [
            ...
            .init(id: "loc.holysep", name: "Church of the Holy Sepulchre",
                  ...
                  associatedEventIDs: [],
                  artworkID: nil, sourceIDs: ["src.eusebius"],
            ...
            .init(id: "loc.constantinople", name: "Hagia Sophia",
                  ...
                  confidence: .confirmed,
                  shortWhyItMatters: "Great Church of Constantinople from 537. Site of the definitive break of 1054 (bull of excommunication laid on the altar).",
            ...
            .init(id: "loc.hebron", name: "Cave of the Patriarchs",
                  ...
                  tradition: .interfaith, categories: [.biblical, .pilgrimage],
                  confidence: .traditional,
                  shortWhyItMatters: "Traditional burial site of Abraham, Isaac, Jacob, Sarah, Rebekah, and Leah. Sacred to Jews, Christians, and Muslims.",
                  visitingHours: nil, associatedEventIDs: [], artworkID: nil,
                  sourceIDs: [], routePackIDs: []),
...
final class SacredAtlasViewModel {
    ...
    // SUGGESTION: Temporal foundation (uncomment UI in filters to enable)
    var selectedEra: ClosedRange<Int>? = nil
    ...
    func attemptOpen(_ location: SacredLocation) -> Bool {
        ...
        if visibleLocations.first(where: { $0.id == location.id }) != nil {
            return true
        }
        isPaywallOpen = true
        return false
    ...
    func toggle(tradition: Tradition) {
        selectedTraditions.toggle(tradition)
    }
    ...
    func toggle(confidence: Confidence) {
        ...
        selectedConfidence.toggle(confidence)
    }
    func clearFilters() {
...
final class PilgrimJourneyViewModel {
    ...
    func startJourney(pack: RoutePack) {
        ...
        location.startSimulating(alongRoute: repo.stops(for: pack))
    }
...
struct SacredAtlasMapView: View {
    ...
    var body: some View {
        ...
        ZStack(alignment: .top) {
            ...
            VStack(spacing: 12) {
                ...
                // SUGGESTION: "On This Ground" proximity banner
                if let nearest = onThisGroundLocation {
                    OnThisGroundBanner(location: nearest)
                        .transition(.move(edge: .top).combined(with: .opacity))
                }
                Spacer()
            }
    ...
    // SUGGESTION: Proximity helper for "On This Ground"
    private var onThisGroundLocation: SacredLocation? {
        guard let user = location.currentCoordinate else { return nil }
        let nearby = vm.filteredLocations
            .map { ($0, $0.distance(from: user)) }
            .filter { $0.1 < 100 } // 100 meters
            .min { $0.1 < $1.1 }
        return nearby?.0
    }
            ...
            if journey.currentSession != nil, journey.stops.count > 1 {
                MapPolyline(coordinates: journey.stops.map(\.coordinate))
                    .stroke(AnnoTheme.gold.opacity(0.9),
                            style: StrokeStyle(lineWidth: 3, dash: [1, 8]))
            }
    ...
    private var topBar: some View {
        HStack(spacing: 10) {
            ...
            // SUGGESTION: Debug tier switcher (remove for production)
            Menu {
                ForEach(EntitlementTier.allCases, id: \.self) { tier in
                    Button(tier.rawValue.capitalized) { entitlements.tier = tier }
                }
            } label: {
                TopIconButton(system: "crown.fill") { }
            }
        }
        .padding(.horizontal)
        .padding(.vertical, 8)
...
struct LocationDetailSheet: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                ...
                whyItMatters
                if !location.categories.isEmpty { chipsRow }
                connectedEvents
                metadataGrid
                sourceButtons // SUGGESTION: quick source access
                actionsRow
                if !entitlements.canReadFullDossier {
                    PaywallCardView(
                        ...
                        specifics: [
                            LocalizedStringKey("2,400+ sacred sites (mock)"),
    ...
    // SUGGESTION: Direct source buttons
    private var sourceButtons: some View {
        let srcs = vm.repo.sources(ids: location.sourceIDs)
        return FlowLayoutHStack(spacing: 6) {
            ForEach(srcs) { src in
                Button {
                    if let url = src.url { showSafariURL = url }
                } label: {
                    Label(src.title, systemImage: "link")
                        .font(AnnoTheme.label(10))
                        .foregroundStyle(AnnoTheme.gold)
                        .padding(.horizontal, 10).padding(.vertical, 6)
                        .background(AnnoTheme.gold.opacity(0.08), in: Capsule())
                        .overlay(Capsule().strokeBorder(AnnoTheme.gold.opacity(0.3)))
                }
                .buttonStyle(.plain)
                .disabled(src.url == nil)
            }
        }
    }
...
private struct StampView: View {
    ...
    var body: some View {
        VStack(spacing: 6) {
            // SUGGESTION: Cryptographic seal visual from Approach 2
            ZStack {
                Circle()
                    .strokeBorder(stamp.tradition.tint, style: StrokeStyle(lineWidth: 1.5, dash: [4]))
                    .frame(width: 58, height: 58)
                VStack(spacing: 2) {
                    Image(systemName: stamp.tradition.symbol)
                        .font(.system(size: 18))
                        .foregroundStyle(stamp.tradition.tint)
                    Text(stamp.localizedStampCode)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            if let next = journey.nextStop {
                ...
                Text(LocalizedStringKey("All stops visited. Consider ending the journey to complete it."))
                    .font(AnnoTheme.serifBody(14))
                    .foregroundStyle(AnnoTheme.textSecondary)
            }
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 14) {
                    Text(L.nearby).font(AnnoTheme.serifTitle(24))
                        .foregroundStyle(AnnoTheme.textPrimary)
                    Text(LocalizedStringKey("Sacred sites within 400 km of your current position, ranked by proximity."))
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 16) {
                ...
                Text(LocalizedStringKey("Unlock the Sacred Atlas"))
                    .font(AnnoTheme.serifTitle(26))
                    .foregroundStyle(AnnoTheme.textPrimary)
                Text(LocalizedStringKey("Explore 2,400+ sacred sites (mock), source-backed place histories, and curated pilgrimage routes."))
    ...
    var body: some View {
        HStack(spacing: 10) {
            Image(systemName: "location.fill")
                .foregroundStyle(AnnoTheme.gold)
            VStack(alignment: .leading, spacing: 2) {
                Text(L.onThisGround).font(AnnoTheme.label(10))
                    .foregroundStyle(AnnoTheme.gold)
                Text(location.name).font(AnnoTheme.serifBody(13).weight(.semibold))
                    .foregroundStyle(AnnoTheme.textPrimary)
            }
            Spacer()
            Image(systemName: "chevron.right")
        ...
        .padding(.horizontal, 14).padding(.vertical, 10)
        .background(AnnoTheme.ink3, in: Capsule())
        .overlay(Capsule().strokeBorder(AnnoTheme.gold.opacity(0.6), lineWidth: 1))
        .padding(.horizontal)
    }
}

// SUGGESTION: Safari wrapper for source citations
...
private struct MapUtilityColumn: View {
    @Environment(SacredAtlasViewModel.self) private var vm
    @Environment(LocationService.self)      private var location
    var body: some View {
        VStack(spacing: 10) {
            Button {
                ...
                Image(systemName: "location.fill")
                    ...
                    .background(AnnoTheme.ink3, in: Circle())
                    .overlay(Circle().strokeBorder(AnnoTheme.hairline))
            }
            Button {
                withAnimation { vm.showTodayOnly.toggle() }
            } label: {
                Image(systemName: vm.showTodayOnly ? "calendar.badge.checkmark" : "calendar")
                    .foregroundStyle(vm.showTodayOnly ? AnnoTheme.ink : AnnoTheme.textPrimary)
                    .frame(width: 42, height: 42)
                    .background(vm.showTodayOnly ? AnnoTheme.gold : AnnoTheme.ink3, in: Circle())
                    .overlay(Circle().strokeBorder(AnnoTheme.hairline))
            }
        }
    }
...
private extension EntitlementTier {
    var headerLabel: LocalizedStringKey {
        switch self {
        case .free:    return LocalizedStringKey("anno.tier.freePreview")
        case .premium: return LocalizedStringKey("anno.tier.premium")
        case .pilgrim: return LocalizedStringKey("anno.tier.pilgrimAnnual")
        }
    }
}
...
#Preview("Passport — With Session") {
    let harness = PreviewHarness(tier: .pilgrim) { PilgrimPassportView() }
    let ctx = harness.container.mainContext
    let s = JourneySession(routePackID: "route.romeSeven", routePackTitle: "Seven Churches of Rome")
    let loc = MockSacredRepository.shared.location(id: "loc.stpeters")!
    s.visitedStopIDs = [loc.id]
    s.stamps = [PassportStamp(location: loc)]
    s.notes = [FieldNote(body: "The dome at first light — silence, then bells.", locationID: loc.id)]
